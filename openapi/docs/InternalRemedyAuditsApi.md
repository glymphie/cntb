# \InternalRemedyAuditsApi

All URIs are relative to *https://api.contabo.com*

Method | HTTP request | Description
------------- | ------------- | -------------
[**RetrieveRemediesAuditsList**](InternalRemedyAuditsApi.md#RetrieveRemediesAuditsList) | **Get** /internal/v1/troubleshooting/remedies/audits | List history about your Data (audit)



## RetrieveRemediesAuditsList

> RemediesAuditListResponse RetrieveRemediesAuditsList(ctx).XRequestId(xRequestId).OrgIds(orgIds).XTraceId(xTraceId).Page(page).Size(size).OrderBy(orderBy).CreationStartTime(creationStartTime).CreationEndTime(creationEndTime).AccountId(accountId).RequestId(requestId).ForeignChangedBy(foreignChangedBy).ChangedBy(changedBy).RemedyId(remedyId).Execute()
...
func main() {
    ...
    remedyId := float32(12345) // float32 | Remedy's id (optional)

    configuration := openapiclient.NewConfiguration()
    api_client := openapiclient.NewAPIClient(configuration)
    resp, r, err := api_client.InternalRemedyAuditsApi.RetrieveRemediesAuditsList(context.Background()).XRequestId(xRequestId).OrgIds(orgIds).XTraceId(xTraceId).Page(page).Size(size).OrderBy(orderBy).CreationStartTime(creationStartTime).CreationEndTime(creationEndTime).AccountId(accountId).RequestId(requestId).ForeignChangedBy(foreignChangedBy).ChangedBy(changedBy).RemedyId(remedyId).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `InternalRemedyAuditsApi.RetrieveRemediesAuditsList``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `RetrieveRemediesAuditsList`: RemediesAuditListResponse
    fmt.Fprintf(os.Stdout, "Response from `InternalRemedyAuditsApi.RetrieveRemediesAuditsList`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiRetrieveRemediesAuditsListRequest struct via the builder pattern
...
[**RemediesAuditListResponse**](RemediesAuditListResponse.md)
